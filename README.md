...
3. 「提供判定開始」を押す
	- 対象行が設定済みで実行範囲が「全行」の場合、自動的に「選択行以降」として開始
4. 途中で止める場合は「停止」を押す
5. 完了後「結果CSV保存」で出力する

アプリ終了時、読み込み済みデータがある場合は実行ファイルと同階層（開発実行時はプロジェクト直下）に `result_autosave.csv` を自動保存します。
次回起動時に同ファイルが存在すれば、確認ダイアログを表示し、同意した場合に前回進捗を復元します。
